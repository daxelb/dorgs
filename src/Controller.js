class Controller {
    ...
    updateMouseLocation(offsetX, offsetY) {
        ...
        let colRow = this.env.grid_map.xyToColRow(this.mouse_x, this.mouse_y);
        this.mouse_c = colRow[0];
        this.mouse_r = colRow[1];
        this.curr_cell = this.env.grid_map.cellAt(this.mouse_c, this.mouse_r)
        ...
        if (this.curr_org != prev_org || this.curr_cell != prev_cell) {
            ...
            if (this.curr_org != null) {
                if (this.highlight_org) {
                    console.log("ayo")
                    this.env.renderer.highlightOrg(this.curr_org);
                } else {
                    this.env.renderer.highlightCell(this.curr_cell);
                }
            }
        }
    }
