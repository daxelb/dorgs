...
class Grid {
    ...
    getAllNeighbors(cell) {
        const adjacent = this.getAdjacent(cell);
        const nonAdjacent = this.getNonAdjacent(cell);
        return adjacent.concat(nonAdjacent);
    }

    getAllEmptyNeighbors(cell) {
        const neighbors = this.getAllNeighbors(cell);
        return neighbors.filter((c) => c.isEmpty());
    }
